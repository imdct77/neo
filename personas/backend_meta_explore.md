# 메타 인덱스 탐색 규칙 (Backend)

> **로드 시점**: 신규 기능·함수·상수를 구현하거나 기존 코드를 수정하기 전.
> 페르소나 `personas/backend.md` §2-0에서 분리된 문서.
> `personas/backend.md`를 로드한 후, 탐색이 필요한 작업 시작 시 이 파일도 함께 로드한다.
...
탐색은 `state/meta/src/be/INDEX.md` 메타 인덱스를 통해 수행한다.
...
| 계층 | 메타 인덱스 경로 |
|:---:|------|
| L3 | `state/meta/src/be/{section}/DETAIL.{stem}.md` |
| L2 | `state/meta/src/be/{section}/DETAIL.md` |
| L1 | `state/meta/src/be/{section}/INDEX.md` |

예: `project/src/be/models/user.py` → {section}=`models`, {stem}=`user`
  → L3: `state/meta/src/be/models/DETAIL.user.md`
  → L2: `state/meta/src/be/models/DETAIL.md`
  → L1: `state/meta/src/be/models/INDEX.md`

예: `project/src/be/recipes/model/recipe.py` → {section}=`recipes/model`, {stem}=`recipe`
  → L3: `state/meta/src/be/recipes/model/DETAIL.recipe.md`
  → L2: `state/meta/src/be/recipes/model/DETAIL.md`
  → L1: `state/meta/src/be/recipes/model/INDEX.md`

```
구현·수정 전 탐색 순서 (모든 경로는 state/meta/src/be/ 기준):

1. state/meta/src/be/INDEX.md 읽기 → 하위 디렉토리 목록 파악 (L1)
2. state/meta/src/be/{section}/INDEX.md 읽기 → 파일 목록 + 공용 함수 확인 (L1)
3. 유사 기능 발견 시:
   a. (필요 시) state/meta/src/be/{section}/DETAIL.md 읽기 → 설계 의도 확인 (L2)
   b. 동일 기능이면 → 그것을 사용한다 (재구현 금지)
   c. 유사 기능이면 → 아래 패턴 적용 검토
4. 상수·변수명은 INDEX.md의 공용 요소 목록에서 확인.
   같은 의미의 상수가 이미 있으면 import해서 사용.
   없을 때만 새로 정의.
5. (파일 수정·재사용 시) 반드시 state/meta/src/be/{section}/DETAIL.{파일명}.md (L3)를 먼저 읽는다:
...
7. 수정 완료 후 state/meta/src/be/{section}/DETAIL.{파일명}.md 갱신 항목을 Task Brief에 포함.
...
모든 메타 파일 경로는 state/meta/src/be/ 아래에 위치한다.

**파일 생성 시 (L3 신규 → L3 상태·내용을 들고 L2 검토):**
8. `--sync`가 state/meta/src/be/{section}/DETAIL.{파일명}.md (L3)를 `[AUTO] TODO` skeleton으로 자동 생성한다.
   LLM의 역할: skeleton의 TODO를 의미 있는 내용(함수 시그니처·의존성·상수·중복 금지)으로 채운다.
9. L3 내용을 기준으로 state/meta/src/be/{section}/DETAIL.md 검토 → 파일 인덱스에 `# {file_path} — 상세` 항목 추가
...
10. L2 변경 내용을 기준으로 state/meta/src/be/{section}/INDEX.md 검토 → 파일 라인 추가
11. 변경된 L1 상태·내용을 들고 상위 state/meta/src/be/의 INDEX.md·DETAIL.md 검토
    → `--sync`가 이 cascade(9~11)를 자동 수행한다.

**파일 삭제 시 (L3 제거 → L3 상태를 들고 L2 검토):**
12. 삭제 전 state/meta/src/be/{section}/DETAIL.{파일명}.md (L3) 확인 → 의존성·Import by 확인
    → 이 파일을 참조하는 다른 코드가 있는지 파악
13. 파일 삭제 후:
    a. state/meta/src/be/{section}/DETAIL.{파일명}.md (L3) 삭제
    b. L3 삭제 상태를 기준으로 state/meta/src/be/{section}/DETAIL.md 검토 → 파일 인덱스에서 항목 제거. 남은 파일 0건이면 L2 삭제 판정
    c. L2 변경 내용을 기준으로 state/meta/src/be/{section}/INDEX.md 검토 → 파일 라인 제거. 남은 파일 0건이면 섹션 삭제 판정
    d. 변경된 L1 상태·내용을 들고 상위 state/meta/src/be/의 INDEX.md·DETAIL.md 검토
...
15. 메타 인덱스의 git 히스토리 탐색:
    `cd {HARNESS_ROOT} && git log -- state/meta/src/be/{section}/`
