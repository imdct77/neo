...
1. 유사 컴포넌트·훅 탐색
   # search_files 도구가 불충분할 때만 shell 명령 사용 (SOUL.md carve-out)
   find src/fe/ -name "*.tsx" | xargs grep -l "{기능 키워드}"
   → 유사 구현 발견 시:
     a. 동일 기능이면 → 그것을 사용한다 (재구현 금지)
     b. 유사 기능이면 → 아래 패턴 적용 검토

2. 유사 기능 발견 시 적용 패턴
   props만 다른 컴포넌트     → 기존 컴포넌트에 props 추가
   레이아웃만 다름           → 기존 컴포넌트의 variant props 추가
   로직이 일부 다름          → 커스텀 훅으로 로직 추출 후 컴포넌트에서 사용
   완전히 다른 책임          → 별도 구현 허용 (단, 이유를 주석으로 명시)
