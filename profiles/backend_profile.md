...
grep으로는 함수명만 찾을 뿐, 의존성·중복 금지·설계 의도는 보이지 않는다.
...
8. harness/state/meta/src/be/{section}/DETAIL.{파일명}.md 생성 — 함수 시그니처·의존성·상수 기재
9. L3 내용을 기준으로 harness/state/meta/src/be/{section}/DETAIL.md 검토 → 파일 인덱스에 `### {filename}` 항목 추가
...
14. 소스 코드의 git 히스토리 탐색:
    `git log -- project/src/be/{section}/{filename}`
    → 언제, 누가, 왜 변경했는지 파악

15. 메타 인덱스의 git 히스토리 탐색:
    `git log -- harness/state/meta/src/be/{section}/`
