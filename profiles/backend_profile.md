...
4. 상수·변수명은 INDEX.md의 공용 요소 목록에서 확인.
   ...
   없을 때만 새로 정의.
5. (파일 수정·재사용 시) 반드시 DETAIL.{파일명}.md (L3)를 먼저 읽는다:
...
파일 생성과 삭제는 **무조건 L2 수정 트리거**다. L3→L2→L1 순으로 전파한다.

**파일 생성 시 (L3 신규):**
8. L3 DETAIL.{파일명}.md 생성 — 함수 시그니처·의존성·상수 기재
9. L2 DETAIL.md 수정 — 파일 인덱스에 `### {filename}` 항목 추가
10. L1 INDEX.md 수정 — 해당 섹션에 파일 라인 추가: `- \`src/be/{section}/{file}\` — {한 줄 목적}`
11. 상위 디렉토리 L2/L1 검토 — 하위 디렉토리 인덱스 갱신 여부 판단

**파일 삭제 시 (L3 제거):**
12. 삭제 전 DETAIL.{파일명}.md (L3) 확인 → 의존성·Import by 확인
    → 이 파일을 참조하는 다른 코드가 있는지 파악
13. 파일 삭제 후:
    a. DETAIL.{파일명}.md (L3) 삭제
    b. L2 DETAIL.md 수정 — 파일 인덱스에서 항목 제거. 남은 파일 0건이면 L2 삭제 판정
    c. L1 INDEX.md 수정 — 파일 라인 제거. 남은 파일 0건이면 섹션 삭제 판정
    d. 상위 디렉토리 L2/L1 검토 — 하위 디렉토리 인덱스 갱신 여부 판단
