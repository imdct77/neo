...
컴포넌트·훅·유틸 파일 생성과 삭제는 **무조건 L2 수정 트리거**다. L3→L2→L1 순으로 전파한다.

**파일 생성 시 (L3 신규):**
8. L3 DETAIL.{파일명}.md 생성 — Props·리렌더·상태흐름·의존성 기재
9. L2 DETAIL.md 수정 — 파일 인덱스에 `### {ComponentName}.tsx` 또는 `### {hookName}.ts` 항목 추가
10. L1 INDEX.md 수정 — 해당 섹션에 파일 라인 추가: `- \`src/fe/{section}/{file}\` — {한 줄 목적}`
