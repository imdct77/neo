...
## 3. 정지 조건 (무한 융합 루프 방지)
- `further_exploration`은 루프를 한 바퀴 더 돌린다(새 후보 생성→재평가). 무한 방지:
  - 사용자 승인 없이는 추가 탐색 안 함(매 바퀴 사용자 게이트).
  - 새 후보가 기존 Pareto를 개선하지 못하면(비지배 추가 실패) "더 나아지지 않음"을 보고하고 멈춤.
  - 예산(바퀴 수·후보 수) 상한 `[설정 후보: 비용]`.
